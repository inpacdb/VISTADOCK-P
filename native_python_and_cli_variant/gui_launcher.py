...
class PipelineGUI:
    def __init__(self, root):
        ...
        style.configure("Bold.TLabel", font=("Helvetica", 13, 'bold'), background="white")
        style.configure("Section.TLabel", font=("Helvetica", 12, 'bold'), background="white") 
        ...
        style.map("Info.TButton", background=[("active", "#0056b3")])
        ...
        self.banner_frame = tk.Frame(self.root, bg="#333333", bd=2, relief="solid")
        ...
        self.info_btn = ttk.Button(self.root, text="ℹ", command=self.toggle_banner, width=5, style="Info.TButton")
        self.info_btn.place(relx=0.98, rely=0.01, anchor="ne")
        
    def toggle_banner(self):
        """Toggle switch for info banner"""
        if self.banner_visible:
            self.banner_frame.place_forget()
            self.banner_visible = False
        else:
            self.banner_frame.place(relx=0.5, rely=0.5, anchor="n")
    ...
    def create_widgets(self):
        ...
        warnings = ("1. Ensure that you have created a conda environment with all the packages required for the pipeline\n    (all the packages and binaries mentioned in the file 'environment.yml')\n"
                    "2. Ensure that you have activated the conda environment for this pipeline: 'conda activate vistadock_p_env'\n"
                    "3. Ensure that 'obabel', 'smina', 'gnina' and 'plip' are in your system PATH.\n"
                    "4. Ensure that your Receptor PDB is fixed before running the pipeline (bond orders, valencies, non-standard residues etc.).\n"
                    "5. Fields marked with an asterisk (*) are mandatory and the pipeline will not run if they are not filled in."
        )
        ...
        self.exh_rapid = self.create_labelled_entry(self.dock_frame, "Exhaustiveness (RAPID): ", 0, 0, default="8")
        self.exh_balanced = self.create_labelled_entry(self.dock_frame, "Exhaustiveness (BALANCED): ", 1, 0, default="16")
        self.exh_ultra = self.create_labelled_entry(self.dock_frame, "Exhaustiveness (ULTRA): ", 2, 0, default="32")

        self.frac_rapid = self.create_labelled_entry(self.dock_frame, "Percentage of top ligands to keep after RAPID: ", 0, 2, default="50")
        self.frac_balanced = self.create_labelled_entry(self.dock_frame, "Percentage of top ligands to keep after BALANCED: ", 1, 2, default="30")
        self.frac_ultra = self.create_labelled_entry(self.dock_frame, "Percentage of top ligands to keep after ULTRA: ", 2, 2, default="100")

        self.single_step_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(self.dock_frame, text="Single Step Docking (Bypass RAPID and BALANCED, run only ULTRA)", variable=self.single_step_var, command=self.toggle_single_step).grid(row=3, column=0, columnspan=4, sticky='w', pady=(10,0))
    ...
    def generate_command(self):
        rec_path = self.receptor_path.get()
        lig_path = self.ligand_path.get()
        ...
        cmd_parts = ["python3 -u vistadockp.py "]
        
        cmd_parts.append(f'--receptor_pdb "{rec_path}"')
        cmd_parts.append(f'--ligand "{lig_path}"')
        cmd_parts.append(f'--output_dir "{out_path}"')
    ...
    def validate_cpu_input(self, var, entry_widget):
        ...
        if value.isdigit():
            if int(value) > max_sys_cpus:
                entry_widget.config(bg="#ce6161", fg="white")
        elif value == "":
            pass
        else:
            entry_widget.config(bg="#ce6161", fg="white")
