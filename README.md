...
**NOTE:** Set *conda* channel priority to flexible if you run into issues during installation using the following command in your terminal:
...
```
**READ THE DISCLAIMERS IN THE GUI CAREFULLY**, input all your values in the provided fields, then click on the `GENERATE COMMAND` button. Once the command has been generated in the text box below the button, click on the `RUN PIPELINE` button, and the pipeline will automatically run on your terminal. 

#### **YOU HAVE TO ENSURE THAT YOUR RECEPTOR PDB, LIGAND FILE AND OUTPUT DIRECTORY ARE ALL PRESENT WITHIN THE SAME FOLDER YOU LAUNCH THE GUI / RUN VISTADOCK-P FROM (only for the Docker version)!!!!!**
