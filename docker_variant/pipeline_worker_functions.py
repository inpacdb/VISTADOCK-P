...
def filter_sdf(input_sdf, output_sdf, fraction):
    ...
    keep_count = max(1, int(len(unique_data) * (fraction/100)))
...
def run_screening_track(scoring_func, base_output_dir, return_dict, config):
    ...
    if config.get('single_step', False):
        log(scoring_func, "Single Step Mode Enabled: Bypassing RAPID and BALANCED steps.", color)
        ...
        exh_rapid = config.get('exh_rapid', 8); frac_rapid = config.get('frac_rapid', 0.5)
        
        log(scoring_func, f"Step 1: RAPID (Exh={exh_rapid}, Keep={frac_rapid}%)", color)
        ...
        exh_balanced = config.get('exh_balanced', 16); frac_balanced = config.get('frac_balanced', 0.3)
        
        log(scoring_func, f"Step 2: BALANCED (Exh={exh_balanced}, Keep={frac_balanced}%)", color)
        ...
        log(scoring_func, f"Step 3: ULTRA (Exh={exh_ultra}, Keep={frac_ultra}%)", color)
