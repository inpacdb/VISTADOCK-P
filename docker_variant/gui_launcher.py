...
class PipelineGUI:
    ...
    def generate_command(self):
        ...
        gpu_input = self.gpu_id_var.get().strip()
        wants_gpu = bool(gpu_input)
        
        if self.cnn_var.get() and not wants_gpu:
            messagebox.showerror("GPU Required!", "GNINA CNN Re-Scoring requires a GPU.\nPlease enter a GPU Device ID (e.g. 0, 1, 2 etc.) or disable CNN Re-Scoring.")
        
        if wants_gpu:
            docker_cmd += " --gpus all"
        
        docker_cmd += " -v $(pwd):/data -u $(id -u):$(id -g) vistadock_p python3 -u vistadockp.py "
        
        cmd_parts = [docker_cmd]
        ...
        actual_gpu_id = gpu_input if wants_gpu else "-1"
        cmd_parts.append(f'--gpu_device {actual_gpu_id}')
